 ...
 * 导致 {@code EventService ↔ HandlerRegistry} 形成构造器循环依赖，Spring 无法创建。
 * <p>
 ...
 *       ↑
 *   EventService（构造器依赖 HandlerRegistry + EventEngine）  ← 本类
 * </pre>
 * 三者均可使用构造器注入，Spring 按 EventEngine → HandlerRegistry → EventService 顺序创建。
 *
 * <h3>使用示例</h3>
 * <pre>{@code
...
public class EventService {
     ...
     * EventEngine（无依赖）→ HandlerRegistry（依赖 EventEngine）→ EventService（依赖两者）。
     *
     * @param handlerRegistry 路由引擎
     * @param engine          Nukkit 桥接引擎
     */
    public EventService(HandlerRegistry handlerRegistry, EventEngine engine) {
