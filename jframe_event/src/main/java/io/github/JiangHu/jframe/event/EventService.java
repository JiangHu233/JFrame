...
/**
 * 事件引擎：对接 Nukkit 底层事件系统，按事件类型精准注册和分发。
 * <p>
 * 本类是 L1 引擎层，只负责"按类型订阅"和"转发给消费者"。
 * <b>优先级排序和独占逻辑由上层 {@code HandlerRegistry} 管理</b>，
 ...
 *     return false; // 非独占（本层返回值无实际意义，独占由 HandlerRegistry 管理）
 * });
 * }</pre>
 *
 * @see EventConsumer
 * @see io.github.JiangHu.jframe.event.routing.HandlerRegistry
 */
public class EventService implements Listener, PluginAware {
     ...
     * <p>
     * 在 Spring 容器初始化阶段，{@code EventBeanPostProcessor} 可能在
     * plugin 设置之前扫描到 {@code @EventHandler} Bean 并调用 subscribe。
