 ...
 *   <li><b>熔断保护</b>：{@link AStarOptions#getMaxSearchNodes()} 防止在无解地图上耗尽资源</li>
 * </ul>
 *
 * <h3>线程安全</h3>
 ...
 * <h3>算法要点</h3>
 * <ul>
 *   <li>使用 {@link PriorityQueue}（按 f = g + h 排序）+ long 坐标编码闭合表</li>
 *   <li>方块查询经 {@link BlockSnapshotCache} 快照缓存，消除同坐标重复世界查询</li>
 ...
 * PathResult result = finder.findPath(level, entity, target, new AStarOptions());
 * if (result.isSuccess()) {
...
public class AStarPathFinder implements PathfindingStrategy {
     ...
     * {@inheritDoc}
     * <p>
     * 接口方法：接受通用 {@link PathfindingConfig}，内部 cast 为 {@link AStarOptions}。
    ...
    public PathResult findPath(Level level, Vector3 start, Vector3 target, PathfindingConfig config) {
        AStarOptions opts = (config instanceof AStarOptions)
                ? (AStarOptions) config
                : new AStarOptions();
    ...
    @Override
    public AStarOptions getDefaultConfig() {
        return new AStarOptions();
    }
    ...
    public PathResult findPath(Level level, Vector3 start, Vector3 target) {
        return findPath(level, start, target, new AStarOptions());
    }
     ...
     */
    public PathResult findPath(Level level, Vector3 start, Vector3 target, AStarOptions options) {
        ...
        return new Search(level, options).run(AStarNode.fromVector(start), AStarNode.fromVector(target));
    }

    /**
     * 单次寻路的可变状态封装，保证 {@link AStarPathFinder} 外层无状态。
     */
    private static final class Search {

        private final Level level;
        private final AStarOptions opts;
        /** 单次搜索的方块快照：同一坐标的重复 getBlock 降为 HashMap 命中 */
        private final BlockSnapshotCache snapshot;
        private final PriorityQueue<AStarNode> open;
        /** 闭合表：long 坐标编码 key（无碰撞、无对象哈希开销） */
        private final HashSet<Long> closed;
        ...
        Search(Level level, AStarOptions opts) {
            this.level = level;
            this.opts = opts;
            this.snapshot = new BlockSnapshotCache(level);
            this.open = new PriorityQueue<>((a, b) -> Double.compare(a.fCost(), b.fCost()));
            this.closed = new HashSet<>();
            this.dirs = opts.isAllowDiagonal() ? DIRS_8 : DIRS_4;
        }

        PathResult run(AStarNode start, AStarNode goal) {
            ...
            while (!open.isEmpty()) {
                ...
                // 惰性删除：已闭合的过时条目直接跳过（long 坐标编码 key）
                if (!closed.add(current.positionKey())) {
                    continue;
                }
                ...
                for (int[] d : dirs) {
                    ...
                    // 切角检测前置：对角线移动时，若两侧正交格均为固体，则禁止穿过夹角。
                    // 仅查 2 格即可短路，避免为注定失败的方向执行昂贵的 findLanding（多格探测）
                    if (diagonal && isCornerBlocked(current, d[0], d[1])) {
                        continue;
                    }
                    AStarNode neighbor = findLanding(current, d[0], d[1]);
                    if (neighbor == null || closed.contains(neighbor.positionKey())) {
                        continue;
                    }
        ...
        /**
         * 失败时根据 {@link AStarOptions#isPartialOnFailure()} 决定返回失败还是部分路径。
         * <p>
         ...
         * 则实体无法穿过它们的夹角。
         */
        private boolean isCornerBlocked(AStarNode cur, int dx, int dz) {
        ...
        private List<AStarNode> reconstruct(AStarNode end) {
            List<AStarNode> path = new ArrayList<>();
            for (AStarNode n = end; n != null; n = n.cameFrom) {
                path.add(n);
            }
