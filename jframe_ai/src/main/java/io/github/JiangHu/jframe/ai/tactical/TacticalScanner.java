...
import io.github.JiangHu.jframe.ai.util.LineOfSight;
 ...
 *       {@link io.github.JiangHu.jframe.ai.navigation.NavigatorManager} 驱动。</li>
...
public class TacticalScanner {
    ...
    public TacticalPosition findCover(Entity self, Vector3 threatPos, double radius) {
        ...
        for (int dx = -r; dx <= r; dx++) {
            for (int dz = -r; dz <= r; dz++) {
                ...
                int by = findStandableY(level, bx, floor(self.y), bz, DEFAULT_ENTITY_HEIGHT);
                ...
                // 关键条件：威胁看不到候选位置
                if (LineOfSight.hasLineOfSight(level, threatEye, candidateEye)) {
                    continue;
                }
     ...
     *           {@link io.github.JiangHu.jframe.ai.tactical.TeamTactics#flankTarget(java.util.List, Entity, double)}，
    ...
    public TacticalPosition findHighGround(Entity self, double radius, int minAdvantage) {
        ...
        for (int dx = -r; dx <= r; dx++) {
            for (int dz = -r; dz <= r; dz++) {
                int bx = floor(self.x) + dx;
                int bz = floor(self.z) + dz;
                int by = findStandableY(level, bx, selfY, bz, DEFAULT_ENTITY_HEIGHT);
                if (by == Integer.MIN_VALUE) {
                    continue;
                }
    ...
    public TacticalPosition findSightPosition(Entity self, Vector3 targetPos, double radius, double idealDistance) {
        ...
        for (int dx = -r; dx <= r; dx++) {
            for (int dz = -r; dz <= r; dz++) {
                ...
                Vector3 candidateEye = new Vector3(candidate.x, candidate.y + EYE_HEIGHT, candidate.z);
                // 关键条件：从候选位置能看到目标
                if (!LineOfSight.hasLineOfSight(level, candidateEye, targetEye)) {
                    continue;
                }
                double distToTarget = horizontalDistance(candidate, targetPos);
                double distToSelf = horizontalDistance(candidate, self);
    ...
    public TacticalPosition findApproximatePosition(Entity self, Vector3 center, double uncertaintyRadius) {
        ...
        for (int dx = -r; dx <= r; dx++) {
            for (int dz = -r; dz <= r; dz++) {
                ...
                int by = findStandableY(level, bx, centerY, bz, DEFAULT_ENTITY_HEIGHT);
                if (by == Integer.MIN_VALUE) {
                    continue;
                }
                Vector3 candidate = new Vector3(bx + 0.5, by, bz + 0.5);
     ...
     */
    private int findStandableY(Level level, int bx, int centerY, int bz, int entityHeight) {
        // 同高
        if (BlockChecks.isStandable(level, bx, centerY, bz, entityHeight)) {
            return centerY;
        }
        // 向上搜索 4 格
        for (int dy = 1; dy <= 4; dy++) {
            if (BlockChecks.isStandable(level, bx, centerY + dy, bz, entityHeight)) {
                return centerY + dy;
            }
        }
        // 向下搜索 4 格
