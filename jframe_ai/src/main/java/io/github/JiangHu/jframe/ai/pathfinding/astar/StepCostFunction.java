 ...
 * };
 *
 * AStarOptions options = new AStarOptions().setStepCostFunction(avoidLava);
 * PathResult result = pathFinder.findPath(level, start, target, options);
 * }</pre>
 *
 * <h3>线程安全</h3>
 * 实现方应保证本函数无状态或线程安全，因为 {@link AStarPathFinder} 可被多线程并发调用。
 *
 * @see AStarOptions#stepCostFunction(StepCostFunction)
 * @see AStarPathFinder
 */
@FunctionalInterface
public interface StepCostFunction {
     ...
     */
    double extraCost(Level level, AStarNode from, AStarNode to, boolean diagonal, AStarOptions options);
}
