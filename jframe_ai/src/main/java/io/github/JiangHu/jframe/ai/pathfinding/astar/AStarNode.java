 ...
 * 可让 {@link AStarPathFinder} 的 A* 主循环更简洁（无需维护多张映射），
...
public class AStarNode {
    ...
    /**
     * 将三维坐标编码为单个 long，用作 A* 闭合表与方块快照缓存的 key。
     * <p>
     * 相比 {@link #hashCode()}（int 打包，存在位折叠碰撞），long 编码在
     * 合法坐标范围内（X/Z ±67M，Y [-64, 447]）无碰撞，且省去集合操作时
     * 的对象哈希计算。
     *
     * @return 坐标编码
     * @see BlockSnapshotCache#encode(int, int, int)
     */
    public long positionKey() {
        return BlockSnapshotCache.encode(x, y, z);
    }
     ...
     * @return 水平距离平方
     */
    public double horizontalDistanceSquared(AStarNode other) {
        double dx = x - other.x;
        double dz = z - other.z;
        return dx * dx + dz * dz;
    }
     ...
     * @return 三维距离
     */
    public double distance3D(AStarNode other) {
        double dx = x - other.x;
        double dy = y - other.y;
        double dz = z - other.z;
     ...
     * @return 节点
     */
    public static AStarNode fromVector(Vector3 pos) {
        return new AStarNode(pos.getFloorX(), pos.getFloorY(), pos.getFloorZ());
    }

    @Override
    public boolean equals(Object o) {
        if (this == o) return true;
        if (!(o instanceof AStarNode other)) return false;
    ...
    public String toString() {
        return "AStarNode{x=" + x + ", y=" + y + ", z=" + z + ", g=" + gCost + ", h=" + hCost + "}";
    }
}
