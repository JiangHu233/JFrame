 ...
 * 行为层（LoopBehavior 等循环行为）与 AiAPI 工厂依赖本接口而非具体实现，
 * 从而可在装配期或运行时切换寻路算法，无需修改调用方代码。
 *
 * <h3>内置策略</h3>
 * <ul>
 *   <li>{@link AStarPathFinder}：<b>A* 全局最优寻路</b>——保证最短路径，单次开销较大，
 ...
 * <pre>{@code
 * // 使用 A* 寻路（精确，保证最优）
 * PathfindingStrategy astar = new AStarPathFinder();
 * PathResult r1 = astar.findPath(level, start, target, new AStarOptions());
 *
 * // 使用贪心寻路（快速，开销极低）
 * PathfindingStrategy greedy = new GreedyPathFinder();
 * PathResult r2 = greedy.findPath(level, start, target, new GreedyOptions());
 *
 * // 行为持有 PathfindingStrategy 引用，装配哪个算法就用哪个
 * LoopBehavior chase = ai.loop(mob)
 *         .to(() -> player)
 *         .strategy(greedy);  // 贪心追逐
 * }</pre>
 *
 * <h3>实现约定</h3>
 * 实现方应保证：
...
public interface PathfindingStrategy {
     ...
     * <pre>{@code
     * AStarOptions opts = (AStarOptions) astar.getDefaultConfig();
    ...
    /**
     * 取消当前正在进行的寻路计算（如果实现支持）。
     * <p>
     * 供行为层在停止/切换目标时调用：实现方可通过 volatile 标志让长搜索尽快
     * 熔断退出（返回 {@link PathResult.Status#CANCELLED}），避免无谓的剩余搜索。
     * 默认空实现（不支持取消的策略直接返回）。
     * <p>
     * 线程约定：本方法可从任意线程调用；实现方应保证标志的内存可见性。
     */
    default void cancel() {
    }
}
