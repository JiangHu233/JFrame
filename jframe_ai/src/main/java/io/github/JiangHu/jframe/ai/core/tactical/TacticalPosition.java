...
public record TacticalPosition(Vector3 position, double score, double distanceToThreat) {
    ...
    /**
     * 将该战术位置转换为 {@link io.github.JiangHu.jframe.ai.pathfinding.astar.AStarNode} 脚部坐标。
     *
     * @return 脚部方块坐标节点
     */
    public io.github.JiangHu.jframe.ai.pathfinding.astar.AStarNode toBlockNode() {
        if (!isPresent()) {
            return null;
        }
        return new io.github.JiangHu.jframe.ai.pathfinding.astar.AStarNode(
