...
public abstract class InventoryView {
     ...
     * @param owner      拥有者组件
     */
    void setSlot(int absRow, int absCol, SlotAppearance appearance, SlotAppearance defaultAppearance,
                 SlotType type, InventoryComponent owner) {
        int index = absRow * COLS + absCol;
        if (index < 0 || index >= size) return;
        slots[index] = new SlotData(appearance, defaultAppearance, type, owner);
    }
     ...
     * 获取格子的默认物品外观（空槽占位物品）。
     * <p>
     * 仅对设置了 {@code defaultAppearance} 的 STORAGE 格子返回非 null。
     * 用于 {@link io.github.JiangHu.jframe.inventory.manager.InventoryManager} 拦截
     * 玩家拿走默认占位物品的交易。
     *
     * @param index 格子序号
     * @return 默认物品外观，未设置时返回 null
     */
    public SlotAppearance defaultAppearance(int index) {
        if (index < 0 || index >= size) return null;
        return slots[index].defaultAppearance();
    }

    /**
     * 窗口是否已注册到玩家（{@code addWindow} 已完成）。
     * <p>
     * 用于 {@link io.github.JiangHu.jframe.inventory.manager.InventoryManager} 防止
    ...
    /**
     * 绑定插件实例（由 {@link io.github.JiangHu.jframe.inventory.manager.InventoryManager} 在打开前注入）。
     *
     * @param plugin 插件实例
     */
