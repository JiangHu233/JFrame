 ...
 * @param defaultAppearance 默认物品外观（仅对 STORAGE 格子有意义：空槽时显示的占位物品，{@code null} 表示不显示）
 * @param type              格子类型
 * @param owner             拥有该格子的组件（事件回调目标）
 */
record SlotData(
        SlotAppearance appearance,
        SlotAppearance defaultAppearance,
        SlotType type,
    ...
    static SlotData empty() {
        return new SlotData(null, null, SlotType.LOCKED, null);
    }
}
