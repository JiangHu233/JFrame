...
public class RenderContext {
    ...
    public void slot(int relRow, int relCol, SlotAppearance appearance, SlotType type) {
        ...
        view.setSlot(absRow, absCol, appearance, null, type, component);
    }

    /**
     * 在相对位置放置物品外观（含默认物品）。
     * <p>
     * 仅对 {@link SlotType#STORAGE} 格子有意义：{@code defaultAppearance} 指定空槽时显示的占位物品。
     * 当玩家放入物品后，占位物品会被真实物品覆盖；取出物品后重新渲染时占位物品会恢复显示。
     *
     * @param relRow            相对行（相对于当前组件）
     * @param relCol            相对列
     * @param appearance        物品外观（{@code null} 表示不设置物品，仅标记类型）
     * @param defaultAppearance 默认物品外观（空槽占位，{@code null} 表示不显示占位物品）
     * @param type              格子类型
     */
    public void slot(int relRow, int relCol, SlotAppearance appearance, SlotAppearance defaultAppearance, SlotType type) {
        int absRow = baseRow + relRow;
        int absCol = baseCol + relCol;
        view.setSlot(absRow, absCol, appearance, defaultAppearance, type, component);
    }
