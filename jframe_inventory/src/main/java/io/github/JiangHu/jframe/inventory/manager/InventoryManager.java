package io.github.JiangHu.jframe.inventory.manager;
...
import io.github.JiangHu.jframe.inventory.view.VirtualInventory;
...
public class InventoryManager implements Listener {
    ...
    /**
     * 绑定插件实例（由 {@link io.github.JiangHu.jframe.inventory.InventoryAPI} 调用）。
     *
     * @param plugin 插件实例
     */
        ...
        for (InventoryAction action : event.getTransaction().getActionList()) {
            ...
            switch (type) {
                ...
                case LOCKED:
                    ...
                    break;
                case STORAGE:
                    // 拦截拿走默认占位物品：如果该槽位设置了 defaultAppearance，
                    // 且玩家正在拿走它（源物品=默认物品，目标=空），取消交易。
                    SlotAppearance defaultAp = view.defaultAppearance(slot);
                    if (defaultAp != null) {
                        Item sourceItem = slotAction.getSourceItem();
                        Item targetItem = slotAction.getTargetItem();
                        Item defaultItem = defaultAp.toItem();
                        if (sourceItem != null && !sourceItem.isNull()
                                && (targetItem == null || targetItem.isNull())
                                && sourceItem.getId() == defaultItem.getId()
                                && sourceItem.getDamage() == defaultItem.getDamage()) {
                            hasNonStorage = true; // 取消交易，阻止拿走占位物品
                            break;
                        }
                    }
