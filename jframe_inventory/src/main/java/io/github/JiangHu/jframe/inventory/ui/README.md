...
StorageBox 支持导出当前物品内容与从数据恢复物品，便于在玩家关闭/打开界面时保存与恢复物品（配合 `jframe_data` 等持久化方案）。
...
| 示例 | 场景 | 关键特性 |
|------|------|---------|
| [`ShopInventoryView`](../../../../../example/inventory/ShopInventoryView.java) | 武器商店 | Button + StorageBox + BorderLayout |
