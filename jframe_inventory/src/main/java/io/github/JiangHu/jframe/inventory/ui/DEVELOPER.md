...
│  ├── syncToInventory(): appearance → Item                            │
│  └── repaint(): render() + sendContents()                           │
│                                                                     │
│  RenderContext (坐标转换：相对 → 绝对)                               │
│     ↓ setSlot(absRow, absCol, appearance, type)                      │
├─────────────────────────────────────────────────────────────────────┤
│                         事件路由层                                    │
│                                                                     │
...
[`SlotData`](view/SlotData.java) 是渲染结果的内部表示，每个格子有两种状态：

| `appearance` | 行为 |
|---|---|
| 非 null | 框架管理外观（BUTTON/DISPLAY/LOCKED），直接覆盖格子 |
| null | STORAGE 格子：完全由玩家管理，框架不干预（保留玩家放入的物品） |
...
**设计决策：STORAGE 格子如何保留玩家物品？**

STORAGE 格子的 `appearance` 为 null，`syncToInventory` 不会对它们调用 `setItem`，因此玩家放入的物品得以完整保留。框架仅在 `appearance != null`（BUTTON/DISPLAY/LOCKED）时覆盖格子：
...
**防抖窗口** = `OPEN_DELAY_TICKS × 50ms + 500ms`（默认约 1 秒），覆盖视图从"开始打开"到"完全弹出"的全过程。
